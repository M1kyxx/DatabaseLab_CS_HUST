...
select c.c_id, c.c_name, sum(partial_property) as total_property
    from client as c left join (
        select c_id, c_name, sum(ifnull(ifnull(p_amount * pro_quantity, 0) + ifnull(i_amount * pro_quantity, 0) + ifnull(f_amount * pro_quantity, 0) + ifnull(pro_income, 0), 0)) as partial_property
            from client
